...
INLINE uint8 FASTCALL S9xAPUGetByteZ (uint8 Address, struct SIAPU * iapu)
{
...
INLINE void FASTCALL S9xAPUSetByteZ (uint8 byte, uint8 Address, struct SIAPU * iapu, struct SAPU * apu)
{
    if (Address >= 0xf0 && iapu->DirectPage == iapu->RAM)
    {
	if (Address == 0xf3)
...
INLINE uint8 FASTCALL S9xAPUGetByte (uint32 Address, struct SIAPU * iapu)
{
    Address &= 0xffff;
...
INLINE void FASTCALL S9xAPUSetByte (uint8 byte, uint32 Address, struct SIAPU * iapu, struct SAPU * apu)
{
    Address &= 0xffff;
