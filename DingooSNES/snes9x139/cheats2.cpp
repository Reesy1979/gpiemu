...
#else
void S9xAddCheat (bool8, bool8, uint32 address, uint8 byte)
{
  if (Cheat.num_cheats < sizeof (Cheat.c) / sizeof (Cheat.c[0]))
  {
    Cheat.c[Cheat.num_cheats].address = address;
    Cheat.c[Cheat.num_cheats].byte    = byte;
    Cheat.c[Cheat.num_cheats].enabled = FALSE;
    Cheat.num_cheats++;
  }
}
#endif
...
void S9xApplyCheat (uint32 which1)
{
    ...
    if (!Cheat.c [which1].saved)
	Cheat.c [which1].saved_byte = S9xGetByte (address, &CPU);
	...
	*(ptr + (address & 0xffff)) = Cheat.c [which1].byte;
	...
	S9xSetByte (address, Cheat.c [which1].byte, &CPU);
...
static int TryToAppendCheat(const char *code, const char *name)
{
  uint32 address;
  uint8  byte;
  const char *error;
  ...
  if(((error = S9xGameGenieToRaw (code, address, byte)) == NULL) || 
     ((error = S9xProActionReplayToRaw (code, address, byte)) == NULL))
  {
    strncpy(Cheat.c[Cheat.num_cheats].name, name, 50);
    Cheat.c[Cheat.num_cheats].name[51] = 0;
...
bool8 S9xLoadCheatFile (const char *filename)
{
  ...
  {
    int i = strlen(buff);
    while(--i >= 0) if(isspace(buff[i])) buff[i] = 0; else break;

    if(buff[0] == ';') {
      if(patchCount > 0) {
        cheatCodeMap[cheatCodeCount++] = patchIndex << 8 | patchCount;
        patchIndex += patchCount;
        patchCount = 0;
        name[0] = 0;
      }
      if(!name[0]) strcpy(name, buff + 1);
      continue;
    }
    if(strlen(buff) >= 6 && strlen(buff) < 16) {
      if(!TryToAppendCheat(buff, name)) continue;
      patchCount++;
    }
  }
  if(patchCount > 0) {
...
{
    if (which < Cheat.num_cheats) return Cheat.c[which].name;
    return 0;
}

bool8 S9xGetCheatEnable(uint32 which)
{
    if (which < Cheat.num_cheats) return Cheat.c[which].enabled;
    return FALSE;
}
#endif
