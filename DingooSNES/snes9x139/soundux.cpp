...
void S9xSetSoundFrequency (int channel, int hertz)
{
	...
	if (Settings.FixFrequency)
	{
		...
		(unsigned long) ((SysDDec)  SoundData.channels[channel].frequency * 0.980);
	}
    }
}
...
//	freq0 = (unsigned long) ((SysDDec) freq0 * 0.985);
...
void S9xSetStereo (bool8 enable)
{
	so.stereo = enable;
}
