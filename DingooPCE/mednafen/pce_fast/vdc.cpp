...
void VDC_RunFrame(const MDFN_Surface *surface, MDFN_Rect *DisplayRect, MDFN_Rect *LineWidths, int skip)
{
	...
	do
	{
		...
		if(!skip)
		{
			static const int ws[3] = { 256, 341, 512 };

			DisplayRect->x = 0;
			DisplayRect->w = ws[vce.dot_clock];
		}
		uint32 line_buffer[2][512];
		...
		for(int chip = 0; chip < VDC_TotalChips; chip++)
		{
			MDFN_ALIGN(8) uint8 bg_linebuf[8 + 512];
			MDFN_ALIGN(8) uint16 spr_linebuf[16 + 512];
			...
			if(VDC_TotalChips == 2)
				...
				target_ptr16 = surface->pixels16 + (frame_counter - 14 - vce.slstart) * surface->pitchinpix;
			...
			if(vdc->display_counter >= (VDS + VSW) && vdc->display_counter < (VDS + VSW + VDW + 1))
			{
				if(vdc->display_counter == (VDS + VSW))
				...
				if(frame_counter >= 14 && frame_counter < (14 + 242))
				{
					...
					if(SHOULD_DRAW)
					{
						static const int xs[3] =  { 24, 38, 96 };

						int32 width = end - start;
						int32 source_offset = 0;
						int32 target_offset = start - (128 + 8 + xs[vce.dot_clock]);
...
//printf("dw: %d, w: %d, s: %d, t: %d\n", DisplayRect->w, width, source_offset, target_offset);

//						if(DisplayRect->w > HW_SCREEN_WIDTH)
//						{
//							if(source_offset > 0) source_offset = 22 - (DisplayRect->w - HW_SCREEN_WIDTH) / 2;
//							width = HW_SCREEN_WIDTH;
//							target_offset = 0;
//						}
						...
						if(width > 0)
						{
							if(target_ptr16) {
								switch(vdc->CR & 0xC0) {
								case 0xC0: MixBGSPR_16BPP(width, bg_linebuf + (vdc->BG_XOffset & 7) + source_offset, spr_linebuf + 0x20 + source_offset, target_ptr16 + target_offset); break;
								case 0x80: MixBGOnly(width, bg_linebuf + (vdc->BG_XOffset & 7) + source_offset, target_ptr16 + target_offset); break;
								case 0x40: MixSPROnly(width, spr_linebuf + 0x20 + source_offset, target_ptr16 + target_offset); break;
								case 0x00: MixNone(width, target_ptr16 + target_offset); break;
								}
							} else {
								switch(vdc->CR & 0xC0) {
								case 0xC0: MixBGSPR(width, bg_linebuf + (vdc->BG_XOffset & 7) + source_offset, spr_linebuf + 0x20 + source_offset, target_ptr + target_offset); break;
								case 0x80: MixBGOnly(width, bg_linebuf + (vdc->BG_XOffset & 7) + source_offset, target_ptr + target_offset); break;
								case 0x40: MixSPROnly(width, spr_linebuf + 0x20 + source_offset, target_ptr + target_offset); break;
								case 0x00: MixNone(width, target_ptr + target_offset); break;
								}
							}
						}
		...
		if(VDC_TotalChips == 2 && SHOULD_DRAW)
			if(frame_counter >= 14 && frame_counter < (14 + 242))
				MixVPC(DisplayRect->w, line_buffer[0] + DisplayRect->x, line_buffer[1] + DisplayRect->x, surface->pixels16 + (frame_counter - 14 - vce.slstart) * surface->pitchinpix);
...
void VDC_RunFrame(const MDFN_Surface *surface, MDFN_Rect *DisplayRect, MDFN_Rect *LineWidths, int skip)
{
 ...
 do
 {
	...
	uint32 line_buffer[2][512];	// For super grafx emulation
	...
	for(int chip = 0; chip < VDC_TotalChips; chip++)
	{
	 ...
	 else
	 {
		if(surface->format.bpp == 16)
		 target_ptr16 = surface->pixels16 + (frame_counter - 14) * DisplayRect->w;
#ifdef CONFIG_SUPPORT_32BPP
		else
		 target_ptr = surface->pixels + (frame_counter - 14) * DisplayRect->w;
#endif
	 }
	...
	if(VDC_TotalChips == 2 && SHOULD_DRAW)
	 ...
	 {
		if(surface->format.bpp == 16)
		 MixVPC(DisplayRect->w, line_buffer[0] + DisplayRect->x, line_buffer[1] + DisplayRect->x, surface->pixels16 + (frame_counter - 14) * DisplayRect->w + DisplayRect->x);
#ifdef CONFIG_SUPPORT_32BPP
		else
		 MixVPC(DisplayRect->w, line_buffer[0] + DisplayRect->x, line_buffer[1] + DisplayRect->x, surface->pixels + (frame_counter - 14) * DisplayRect->w + DisplayRect->x);
#endif
	 } 
 ...
 /*if(!skip)
 {
	for(int y = DisplayRect->y; y < DisplayRect->y + DisplayRect->h; y++)
	{
	 if(!LineWidths[y].w)
	 {
		...
		if(surface->format.bpp == 16)
		 target_ptr16 = surface->pixels16 + y * DisplayRect->w;
		else
		 target_ptr = surface->pixels + y * DisplayRect->w;
