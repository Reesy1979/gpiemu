...
static uint32 GetSectorCount(CDRFILE_TRACK_INFO *track)
{
 ...
 if(track->DIFormat == DI_FORMAT_AUDIO)
 {
  ...
  {
	  long fileSize = track->fp->size();

   //printf("%d %d %d\n", (int)stat_buf.st_size, (int)track->FileOffset, (int)stat_buf.st_size - (int)track->FileOffset);
   if(track->SubchannelMode)
    return((fileSize - track->FileOffset) / (2352 + 96));
   else
    return((fileSize - track->FileOffset) / 2352);
  }
 }
 else
 {
	 long fileSize = track->fp->size();
  return((fileSize - track->FileOffset) / DI_Size_Table[track->DIFormat]);
 }
...
static bool ParseTOCFileLineInfo(CDRFILE_TRACK_INFO *track, const int tracknum, const char *filename, const char *binoffset, const char *msfoffset, const char *length)
{
 ...
 if(NULL == (track->fp = IoSys::open(efn.c_str())))
 {
	 // TODO: remove errno and use info from IoSys::open
  ErrnoHolder ene(errno);

  snprintf(MDFN_cdErrorStr, sizeof(MDFN_cdErrorStr), "Error opening referenced file:\n%s", filename);
...
static CDRFile *ImageOpen(const char *path)
{
 ...
 bool IsTOC = FALSE;
  ...
  if(!(fp = fopen(path, "rb")))
  {
   ...
   snprintf(MDFN_cdErrorStr, sizeof(MDFN_cdErrorStr), "Error opening CUE/TOC file");
  ...
  while(fgets(linebuf, 512, fp) > 0)
  {
   ...
   else // now for CUE sheet handling
   {
    if(!strcasecmp(cmdbuf, "FILE"))
    {
     ...
     if(NULL == (TmpTrack.fp = IoSys::open(efn.c_str())))
     {
      ErrnoHolder ene(errno);
...
int cdrfile_read_raw_sector(CDRFile *p_cdrfile, uint8 *buf, int32 lba)
{
 ...
 if(p_cdrfile->p_cdio)
 {
  ...
  else if(p_cdrfile->CanMMC)
  {
   ...
   if(mmc_run_cmd (p_cdrfile->p_cdio, MMC_TIMEOUT_DEFAULT,
                      ...
                      buf))
   {
	   MDFN_printf("MMC read error, sector %d\n", lba);
    memset(buf, 0, 2352);
    return(0);
   }
  }
  else // else to if(p_cdrfile->CanMMC)
  {
   ...
   else // Audio sector
   {
    ...
    {
    	MDFN_printf("Audio read error, sector %d\n", lba);
     memset(buf, 0, 2352);
     return(0);
    }
   }
 ...
 if(qbuf[0] & 0x40)
 {
  ...
  for(int i = 0; i < 2352 + 96; i++)
  {
   ...
   {
	   MDFN_printf("Mismatch at %d, %d: %02x:%02x; ", lba, i, dummy_buf[i], buf[i]);
    any_mismatch = TRUE;
   }
  }
  if(any_mismatch)
...
static void MakeSubQ(const CDRFile *p_cdrfile, uint32 lba, uint8 *SubPWBuf)
{
 ...
 if(!track_found)
 {
	 MDFN_printf("MakeSubQ error for sector %u!", lba);
