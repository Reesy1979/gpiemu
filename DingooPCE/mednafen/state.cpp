 ...
 while(sf->size || sf->name)
 {
  ...
  while(sub_sf->size || sub_sf->name)
  {
   if(sf != sub_sf)
   {
    ...
    {
    	MDFN_printf("Duplicate state variable name: %.32s\n", sf->name);
    }
   }
   sub_sf++;
  }
...
static bool SubWrite(StateMem *st, SFORMAT *sf, int data_only, const char *name_prefix = NULL)
{
 ...
 while(sf->size || sf->name)	// Size can sometimes be zero, so also check for the text name.  These two should both be zero only at the end of a struct.
 {
  ...
  if(!data_only)
  {
   ...
   if(slen >= 255)
   {
	   MDFN_printf("Warning:  state variable name possibly too long: %s %s %s %d\n", sf->name, name_prefix, nameo, slen);
    slen = 255;
   }
   ...
   for(section = sections.begin(); section != sections.end(); section++)
   {
    ...
    while(smem_read(st, (uint8 *)sname, 32) == 32)
    {
     ...
     if(!strncmp(sname, section->name, 32))
     {
      ...
      {
    	  MDFN_printf("Error reading chunk: %s\n", section->name);
       return(0);
      }
      found = 1;
      break;
     ...
     else
     {
      ...
      {
    	  MDFN_printf("Chunk seek failure");
       return(0);
      }
     }
    }
    ...
    {
    	MDFN_printf("Reverse seek error");
     return(0);
    }
    if(!found && !section->optional) // Not found.  We are sad!
    {
    	MDFN_printf("Section missing:  %.32s\n", section->name);
     return(0);
    }
   }
  }
...
#ifdef _DINGOO
bool MDFN_DumpToStateFile(const char *filename, const void *data, uint64 length)
{
  if(state_fopen("wb")) 
  {
    state_fwrite(data, length);
    state_fclose();
    return true;
  }
  return false;
...
#ifdef _DINGOO
	if(!MDFN_DumpToStateFile(fname, st.data, st.len))
#else
...
int MDFNSS_Load(const char *fname, const char *suffix)
{
  ...
  if(fname && state_fopen("rb")) 
  {
    StateMem st;
    memset(&st, 0, sizeof(StateMem));

    int oldpos = state_ftell();
    state_fseek(0, SEEK_END);
    st.len = state_ftell();
    state_fseek(oldpos, SEEK_SET);

    if(st.len >= 32 && (st.data = (uint8 *)malloc(st.len))) 
    {
      if(state_fread(st.data, st.len) == (int32)st.len)
      {
        if(MDFNSS_LoadSM(&st, 1, 0)) result = 1;
      }
      free(st.data);
    }
    state_fclose();
