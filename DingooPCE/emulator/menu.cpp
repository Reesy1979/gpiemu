...
static void Menu_InputSets_Texts()
{
	...
	menu::set_item_optstr(IS_KEY_RUN,         "Button Run:          %s", dingooButtons[settings.pad_config[0]]);
	menu::set_item_optstr(IS_KEY_SEL,         "Button Select:       %s", dingooButtons[settings.pad_config[1]]);
	menu::set_item_optstr(IS_KEY_I,           "Button I:            %s", dingooButtons[settings.pad_config[2]]);
	menu::set_item_optstr(IS_KEY_II,          "Button II:           %s", dingooButtons[settings.pad_config[3]]);
		...
		switch(menu::update(IS_count, menufocus))
		{
			...
			case MA_DECREASE:
				...
				{		
					...
					case IS_KEY_RUN: case IS_KEY_SEL: case IS_KEY_I: case IS_KEY_II:
						if(--settings.pad_config[menufocus - IS_KEY_RUN] > 12) settings.pad_config[menufocus - IS_KEY_RUN] = 12;
						break;
				}
				Menu_InputSets_Texts();
				break;
			...
			case MA_INCREASE:
				...
				{		
					...
					case IS_KEY_RUN: case IS_KEY_SEL: case IS_KEY_I: case IS_KEY_II:
						if(++settings.pad_config[menufocus - IS_KEY_RUN] > 12) settings.pad_config[menufocus - IS_KEY_RUN] = 0;
						break;
				}
				Menu_InputSets_Texts();
				break;
...
static void Menu_About()
{
	...
	menu::set_item_text(menuCount++, "======================================");
	menu::set_item_text(menuCount++, "$");
	menu::set_item_text(menuCount++, "PC Engine, TurboGrafx-16 emulator for ");
	menu::set_item_text(menuCount++, "Dingoo A320 native OS.                ");
	menu::set_item_text(menuCount++, "$");
