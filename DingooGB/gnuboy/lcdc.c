...
void stat_trigger()
{
	...
	gbhw_interrupt(flag, IF_STAT);
}

void stat_write(byte b)
{
	R_STAT = (R_STAT & 0x07) | (b & 0x78);
	if (!gbhw.cgb && !(R_STAT & 2)) /* DMG STAT write bug => interrupt */
		gbhw_interrupt(IF_STAT, IF_STAT);
	stat_trigger();
}
...
static void stat_change(int stat)
{
	...
	if (stat != 1) gbhw_interrupt(0, IF_VBLANK);
	/* gbhw_interrupt((stat == 1) ? IF_VBLANK : 0, IF_VBLANK); */
	stat_trigger();
}
...
void lcdc_trans()
{
	...
	while (C <= 0)
	{
		...
		{
		case 1:
			if (!(gbhw.ilines & IF_VBLANK))
			{
				C += 218;
				gbhw_interrupt(IF_VBLANK, IF_VBLANK);
				break;
			}
			if (R_LY == 0)
			{
