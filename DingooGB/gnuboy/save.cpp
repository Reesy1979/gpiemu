...
extern "C" void loadstate(FILE *f)
{
	int i, j;
	byte buf[4096];
	un32 (*header)[2] = (un32 (*)[2])buf;
	un32 d;
	int irl = gbhw.cgb ? 8 : 2;
	int vrl = gbhw.cgb ? 4 : 2;
...
extern "C" void savestate(FILE *f)
{
	int i;
	byte buf[4096];
	un32 (*header)[2] = (un32 (*)[2])buf;
...
void system_save_state(void)
{
	...
	state_fseek(0, SEEK_SET);
	state_fwrite(buf, 4096*1);
	
	state_fseek(iramblock<<12, SEEK_SET);
	state_fwrite(ram.ibank, 4096*irl);
	
	state_fseek(vramblock<<12, SEEK_SET);
	state_fwrite(lcd.vbank, 4096*vrl);
	
	state_fseek(sramblock<<12, SEEK_SET);
	state_fwrite(ram.sbank, 4096*srl);
}
...
void system_load_state(void)
{
	...
	state_fseek(iramblock<<12, SEEK_SET);
	state_fread(ram.ibank, 4096*irl);
	
	state_fseek(vramblock<<12, SEEK_SET);
	state_fread(lcd.vbank, 4096*vrl);
	
	state_fseek(sramblock<<12, SEEK_SET);
	state_fread(ram.sbank, 4096*srl);
}
