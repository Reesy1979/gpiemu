...
PICO_INTERNAL int Load_ISO(const char *iso_name, int is_bin)
{
	...
	lprintf("Track 0 - %02d:%02d:%02d DATA", Tracks[0].MSF.M, Tracks[0].MSF.S, Tracks[0].MSF.F);
	...
	for (num_track = 2, i = 0; i < 100; i++)
	{
		...
		for (j = 0; j < sizeof(exts)/sizeof(char *); j++)
		{
			...
			if (tmp_file)
			{
				...
				{
					lprintf("Error track %i: rate %i", index, Tracks[index].KBtps);
#if !DONT_OPEN_MANY_FILES
					fclose(tmp_file);
#else
					free(tmp_file);
				...
				lprintf("Track %i: %s - %02d:%02d:%02d len=%i AUDIO", index, tmp_name, Tracks[index].MSF.M,
...
PICO_INTERNAL int FILE_Play_CD_LBA(void)
{
	...
	lprintf("Play track #%i", Pico_mcd->scd.Cur_Track);
