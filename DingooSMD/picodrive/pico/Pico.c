...
static void PicoRunZ80Simple(int line_from, int line_to)
{
  ...
  if (!(PicoOpt & PicoOpt_enable_z80) || Pico.m.z80Run == 0) line_to_r = 0;
  else {
...
static int PicoFrameSimple(void)
{
  ...
  if (!(PicoOpt & PicoOpt_enable_ym2612_dac) && PsndOut) {
  ...
  if (Pico.m.z80Run && (PicoOpt & PicoOpt_enable_z80))
    z80_int();
