...
void PsndRerate(int preserve_state)
{
  ...
  if (PicoMCD & 1) {
    ...
    PicoOpt |= PicoOpt_stereo_sound; // force stereo
  }
  ...
  // set mixer
  PsndMix_32_to_16l = (PicoOpt & PicoOpt_stereo_sound) ? mix_32_to_16l_stereo : mix_32_to_16_mono;
}
...
{
  int pos, len;
  int do_dac = PsndOut && (PicoOpt & PicoOpt_enable_ym2612_dac) && *ym2612_dacen;
//  int do_pcm = PsndOut && (PicoMCD&1) && (PicoOpt&0x400);
  ...
  if (do_dac) {
    ...
    if(PicoOpt & PicoOpt_stereo_sound) {
  ...
  if (do_pcm) {
    int *d = PsndBuffer;
    d += (PicoOpt & PicoOpt_stereo_sound) ? pos*2 : pos;
    pcm_update(d, len, 1);
  }
#endif
}
...
PICO_INTERNAL int PsndRender(int offset, int length)
{
  ...
  int stereo = (PicoOpt & PicoOpt_stereo_sound) >> 3;
  // emulating CD && PCM option enabled && PCM chip on && have enabled channels
  int do_pcm = (PicoMCD&1) && (PicoOpt & PicoOpt_enable_cd_pcm) && (Pico_mcd->pcm.control & 0x80) && Pico_mcd->pcm.enabled;
  offset <<= stereo;
  ...
  // PSG
  if (PicoOpt & PicoOpt_enable_sn76496)
...
//printf("active_chs: %02x\n", buf32_updated);
  ...
  if ((PicoMCD & 1) && (PicoOpt & PicoOpt_enable_cd_cdda) && !(Pico_mcd->s68k_regs[0x36] & 1) && (Pico_mcd->scd.Status_CDC & 1))
...
{
  KernelLogC("z80_init 1\n");
#if defined(_USE_MZ80)
  struct mz80context z80;
  KernelLogC("z80_init 2\n");
  // z80
  mz80init();
  KernelLogC("z80_init 3\n");
  // Modify the default context
  mz80GetContext(&z80);
  KernelLogC("z80_init 4\n");
  // point mz80 stuff
