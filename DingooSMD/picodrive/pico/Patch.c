...
static int TryToAppendCheat(const char *code, const char *name)
{
  ...
  if(!PicoPatches) array_len = 0;

  if (strlen(code) > 11 || strlen(code) < 8) return 0;

  decode(code, &pt);
  if (pt.addr == (unsigned int)-1 || pt.data == (unsigned short)-1) return 0;
  ...
  strcpy(PicoPatches[PicoPatchCount].code, code);
  strncpy(PicoPatches[PicoPatchCount].name, name, 51);
  PicoPatches[PicoPatchCount].name[51] = 0;
  PicoPatches[PicoPatchCount].active = 0;
  PicoPatches[PicoPatchCount].addr = pt.addr;
  PicoPatches[PicoPatchCount].data = pt.data;
