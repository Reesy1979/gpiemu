...
static void DmaSlow(int len)
{
  ...
  if(Pico.m.scanline != -1) {
    ...
    if ((PicoMCD&1) && (PicoOpt & PicoOpt_cd_perfect_sync)) SekCyclesBurn(CheckDMA());
...
PICO_INTERNAL_ASM void PicoVideoWrite(unsigned int a,unsigned short d)
{
  ...
  if (a==0x00) // Data port 0 or 2
  {
    ...
    else
    {
      ...
      if(!(pvid->status&8) && (pvid->reg[1]&0x40) && Pico.m.scanline!=-1 && !(PicoOpt & PicoOpt_disable_vdp_fifo)) // active display, accurate mode?
      {
...
PICO_INTERNAL_ASM unsigned int PicoVideoRead(unsigned int a)
{
  ...
  if (a==0x04) // control port
  {
    ...
    d=pv->status;
    if (PicoOpt & PicoOpt_alt_renderer) d|=0x0020; // sprite collision (Shadow of the Beast)
