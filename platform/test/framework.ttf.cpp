...
int TTF_Init(void)
{
	...
	if ( error ) {
		debug:printf("Couldn't init FreeType engine\n");
		return(-1);
	}
	return(0);
}
...
TTF_Font *TTF_OpenFont(const char *file, int ptsize)
{
	...
	if ( error ) {
		debug:printf("Couldn't load font file\n");
		free(font);
		return(NULL);
	}
	error = TT_New_Glyph(font->face, &font->glyph);
	if ( error ) {
		debug:printf("Couldn't create glyph container\n");
		TTF_CloseFont(font);
		return(NULL);
	}
	error = TT_New_Instance(font->face, &font->inst);
	if ( error ) {
		debug:printf("Couldn't create font instance\n");
		TTF_CloseFont(font);
		return(NULL);
	}
	...
	if ( error ) {
		debug:printf("Couldn't set font resolution\n");
		TTF_CloseFont(font);
		return(NULL);
	}
	error = TT_Set_Instance_CharSize(font->inst, ptsize*64);
	if ( error ) {
		debug:printf("Couldn't set font size\n");
		TTF_CloseFont(font);
		return(NULL);
	}
	...
	if ( i == n ) {
		debug:printf("Font doesn't have a Unicode mapping\n");
		TTF_CloseFont(font);
		return(NULL);
	}
...
*/
gfx_texture *TTF_RenderText_Solid(TTF_Font *font,
...
*/
gfx_texture *TTF_RenderUTF8_Solid(TTF_Font *font,
				const char *text, gfx_color fg)
{
	gfx_texture *textbuf;
	Uint16 *unicode_text;
...
gfx_texture *TTF_RenderUNICODE_Solid(TTF_Font *font,
...
gfx_texture *TTF_RenderGlyph_Solid(TTF_Font *font, Uint16 ch, gfx_color fg)
{
	gfx_texture *textbuf;
	SDL_Palette *palette;
	Uint8 *src, *dst;
...
/* Convert the Latin-1 text to UNICODE and render it
*/
gfx_texture *TTF_RenderText_Shaded(TTF_Font *font,
...
/* Convert the UTF-8 text to UNICODE and render it
*/
gfx_texture *TTF_RenderUTF8_Shaded(TTF_Font *font,
				const char *text, gfx_color fg, gfx_color bg)
{
	gfx_texture *textbuf;
	Uint16 *unicode_text;
...
gfx_texture *TTF_RenderUNICODE_Shaded(TTF_Font *font,
...
gfx_texture *TTF_RenderGlyph_Shaded(TTF_Font *font,
...
/* Convert the Latin-1 text to UNICODE and render it
*/
gfx_texture *TTF_RenderText_Blended(TTF_Font *font,
				const char *text, gfx_color fg)
{
	gfx_texture *textbuf;
	Uint16 *unicode_text;
...
/* Convert the UTF-8 text to UNICODE and render it
*/
gfx_texture *TTF_RenderUTF8_Blended(TTF_Font *font,
				const char *text, gfx_color fg)
{
	gfx_texture *textbuf;
	Uint16 *unicode_text;
...
gfx_texture *TTF_RenderUNICODE_Blended(TTF_Font *font,
				const Uint16 *text, gfx_color fg)
{
	int xstart, width;
	int w, h;
...
gfx_texture *TTF_RenderGlyph_Blended(TTF_Font *font, Uint16 ch, gfx_color fg)
{
	gfx_texture *textbuf;
	Uint32 alpha_table[] = {
		(255<<24), (128<<24), (64<<24), (32<<24), 0, 0, 0, 0
