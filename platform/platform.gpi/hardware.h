...
static gfx_color gfx_color_r(gfx_color c) 
{
	return (c & (0xF8<<8))>>8;
}

static gfx_color gfx_color_g(gfx_color c) 
{
	return (c & (0xFC<<3))>>3;
}

static gfx_color gfx_color_b(gfx_color c) 
{
	return (c & 0xF8>>3)<<3;
}
...
#define FSYS_DIR_FLAG   0x10
#define FSYS_NEED_PARENT
...
namespace video 
{
	...
	//	textures
	gfx_texture* tex_allocate32(int w, int h);
	gfx_texture* tex_allocate16(int w, int h);
	gfx_texture* tex_allocate8(int w, int h);
	...
	//	fonts
	gfx_font*        font_load(const char* path, gfx_color key, int font_size);
	void             font_size(gfx_font* font, char *str, int *height, int *width);
	...
	void             fill_rect(int x, int y, int w, int h, unsigned char *rgba);
	void             mult_rect(int x, int y, int w, int h, gfx_color color);
	void 		 draw_rgba(int x, int y, unsigned char* rgba, int w, int h);
	...
	void             print(int x, int y, const char* text, gfx_color color, gfx_font *font);

	//	additional
	void 		 clear(gfx_color color);
	void 		 clear_all(bool with_flip);
	void 		 bilinear_scale2(gfx_color* src, int old_w, int old_h, int old_pitch, gfx_color* dst, int new_w, int new_h, int new_pitch);
	void 		 bilinear_scale(gfx_color* src, int old_w, int old_h, int old_pitch, gfx_color* dst, int new_w, int new_h, int new_pitch);
	void             bilinear_scale(unsigned char* src, int old_w, int old_h, int old_pitch, unsigned char* dst, int new_w, int new_h, int new_pitch);
};
...
namespace sound 
{
	...
	void 		fill_s8(int index, char *src, int count);
