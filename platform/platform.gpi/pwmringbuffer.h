...
//
#ifndef _circle_sound_pwmringbuffer_h
#define _circle_sound_pwmringbuffer_h
...
class CPWMRingBuffer : public CSoundBaseDevice	/// Low level access to the PWM sound device
{
public:
	...
	CPWMRingBuffer (CInterruptSystem *pInterrupt,
			     unsigned	       nSampleRate = 44100,
			     unsigned	       nChunkSize  = 4096,
			     int (*userCallback)(short *buffer, int samplecount)=0);

	virtual ~CPWMRingBuffer (void);
...
private:
	...
	u32 *m_pDMABuffer[4];
	u8 *m_pControlBlockBuffer[4];
	TDMAControlBlock *m_pControlBlock[4];
