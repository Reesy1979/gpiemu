...
static int (*UserMixSamples)(short *buffer, int samplecount) = 0;

CPWMRingBuffer::CPWMRingBuffer (CInterruptSystem *pInterrupt,
					  unsigned	    nSampleRate,
					  unsigned	    nChunkSize,
					  int (*userCallback)(short *buffer, int samplecount))
:	CSoundBaseDevice (SoundFormatSigned16,
			  0, nSampleRate,
			  CMachineInfo::Get ()->ArePWMChannelsSwapped ()),
	m_pInterruptSystem (pInterrupt),
	m_nChunkSize (nChunkSize),
	m_nRange ((CLOCK_RATE / nSampleRate)*2),
	...
	m_nDMAChannel (CMachineInfo::Get ()->AllocateDMAChannel (DMA_CHANNEL_LITE))
{
	...
	UserMixSamples = userCallback;
	
	// setup and concatenate DMA buffers and control blocks
	SetupDMAControlBlock (0);
	SetupDMAControlBlock (1);
	SetupDMAControlBlock (2);
	SetupDMAControlBlock (3);
	m_pControlBlock[0]->nNextControlBlockAddress = BUS_ADDRESS ((uintptr) m_pControlBlock[1]);
	m_pControlBlock[1]->nNextControlBlockAddress = BUS_ADDRESS ((uintptr) m_pControlBlock[2]);
	m_pControlBlock[2]->nNextControlBlockAddress = BUS_ADDRESS ((uintptr) m_pControlBlock[3]);
	m_pControlBlock[3]->nNextControlBlockAddress = BUS_ADDRESS ((uintptr) m_pControlBlock[0]);
...
CPWMRingBuffer::~CPWMRingBuffer (void)
{
	...
	delete [] m_pControlBlockBuffer[2];
	m_pControlBlockBuffer[2] = 0;
	delete [] m_pControlBlockBuffer[3];
	m_pControlBlockBuffer[3] = 0;

	delete [] m_pDMABuffer[0];
	m_pDMABuffer[0] = 0;
	delete [] m_pDMABuffer[1];
	m_pDMABuffer[1] = 0;
	delete [] m_pDMABuffer[2];
	m_pDMABuffer[2] = 0;
	delete [] m_pDMABuffer[3];
	m_pDMABuffer[3] = 0;
	
}

u32 *CPWMRingBuffer::AudioBufferGet(u32 buffer)
{
	return m_pDMABuffer[buffer];
}

u32 CPWMRingBuffer::AudioBufferGetCurrIdx(void)
{
	return m_nBuffer;
}

boolean CPWMRingBuffer::Start (void)
{
	assert (m_State == PWMSoundIdle);
...
void CPWMRingBuffer::Cancel (void)
{
	m_SpinLock.Acquire ();
...
boolean CPWMRingBuffer::IsActive (void) const
{
	return m_State != PWMSoundIdle ? TRUE : FALSE;
}

unsigned CPWMRingBuffer::GetChunk (u32 *pBuffer, unsigned nChunkSize)
{
	return UserMixSamples((short*)pBuffer, nChunkSize);
}

boolean CPWMRingBuffer::GetNextChunk (void)
{
	...
	m_nBuffer = m_nNextBuffer;
	
	if(m_nNextBuffer+1>3) m_nNextBuffer=0;
	else m_nNextBuffer++;
...
void CPWMRingBuffer::RunPWM (void)
{
	PeripheralEntry ();

	boolean bOK = m_Clock.StartRate (CLOCK_RATE);
	assert (bOK);
...
void CPWMRingBuffer::StopPWM (void)
{
	PeripheralEntry ();
...
void CPWMRingBuffer::InterruptHandler (void)
{
	assert (m_State != PWMSoundIdle);
	assert (m_nDMAChannel <= DMA_CHANNEL_MAX);
...
void CPWMRingBuffer::InterruptStub (void *pParam)
{
	CPWMRingBuffer *pThis = (CPWMRingBuffer *) pParam;
	assert (pThis != 0);

	pThis->InterruptHandler ();
}

void CPWMRingBuffer::SetupDMAControlBlock (unsigned nID)
{
	assert (nID <= 3);
