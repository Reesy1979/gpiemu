...
int anaglyph_merge_frames;

/* Run the virtual console emulation for one frame */
int system_frame(int skip_render)
{
  ...
  if(sms.glasses_3d)
  {
    if(anaglyph_merge_frames) skip_render = 0;
  }
  ...
  /* 3D glasses */
  int flip_framebuffer = !skip_render;

  if(sms.glasses_3d)
  {
    flip_framebuffer = anaglyph_merge_frames;
    if(!skip_render) anaglyph_merge_frames ^= 1;
  }
