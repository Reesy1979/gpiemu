...
void set_config(const char *ext)
{
	int i;
    ...
    if(!strcmp(ext, ".sg") || !strcmp(ext, ".SG"))
    {
	...
	if(!strcmp(ext, ".rom") || !strcmp(ext, ".ROM") || !strcmp(ext, ".col") || !strcmp(ext, ".COL"))
    {
        ...
        memcpy(coleco.rom, ColecoVision_BIOS, 8192);
    }
...
int system_load_rom(const char *filename)
{
    int i;
    ...
    if(fw::zip::check(filename))
    {
		...
		{	
			...
			{
				cart.rom = (uint8 *)malloc(info.uncompressed_size);
				if(cart.rom)
				{
					cart.size = (size_t)info.uncompressed_size;
					if(cart.size < 0x4000) cart.size = 0x4000;

					if(unzOpenCurrentFile(fd) == UNZ_OK)
					{
						if((uLong)unzReadCurrentFile(fd, cart.rom, cart.size) == info.uncompressed_size)
						{						
							result = true;
						}
    ...
    } else 
    {
	    ...
	    if (cart.size < 0x4000) cart.size = 0x4000;
	    cart.rom = (uint8 *) malloc(cart.size);
