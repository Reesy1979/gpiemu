...
void (*cpu_writemem16)(uint16 address, int data);
void (*cpu_writeport16)(uint8 port, uint8 data);
uint8 (*cpu_readport16)(uint8 port);
 ...
 ***************************************************************/
INLINE void RM16( UINT16 addr, PAIR *r )
{
  r->b.l = RM(addr);
 ...
 ***************************************************************/
INLINE void WM16( UINT16 addr, PAIR *r )
{
  WM(addr, r->b.l);
...
#define ADD(value)                        \
{                                \
  UINT16 ah = AFD & 0xff00;                  \
  UINT8 res = (UINT8)((ah >> 8) + value);          \
  F = SZHVC_add[ah | res];                  \
  A = res;                          \
}
...
#define ADC(value)                        \
{                                \
  UINT16 ah = AFD & 0xff00, c = AFD & 1;            \
  UINT8 res = (UINT8)((ah >> 8) + value + c);        \
...
OP(op,d3) { UINT16 n = ARG() | (A << 8); OUT( n, A ); WZ_L = ((n & 0xff) + 1) & 0xff;  WZ_H = A;           } /* OUT  (n),A       */
...
OP(op,db) { UINT16 n = ARG() | (A << 8); A = IN( n ); WZ = n + 1;                                          } /* IN   A,(n)       */
...
void z80_init(int index, int clock, const void *config, int (*irqcallback)(int))
{
  ...
  Z80.daisy = (const z80_irq_daisy_chain *) config;
