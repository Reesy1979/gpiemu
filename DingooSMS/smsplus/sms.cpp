...
static void writemem_mapper_none(uint16 offset, int data)
{
    cpu_writemap[offset >> 10][offset & 0x03FF] = data;
}

static void writemem_mapper_sega(uint16 offset, int data)
{
    if(offset >= 0xFFFC)
    {
...
static void writemem_mapper_codies(uint16 offset, int data)
{
    if (offset == 0x0000)
    {
        mapper_16k_w(1, data);
...
static void writemem_mapper_korea_msx(uint16 offset, int data)
{
    if (offset <= 0x0003)
    {
...
static void writemem_mapper_korea(uint16 offset, int data)
{
    if (offset == 0xA000)
    {
        mapper_16k_w(3, data);
...
void mapper_8k_w(uint16 address, int data)
{
    int i;
...
void mapper_16k_w(uint16 address, int data)
{
    int i;
    ...
    switch (address)
    {
        case 0: /* control register (SEGA mapper) */
        {
            if(data & 0x08)
            {
                ...
                uint16 offset = (data & 0x04) ? 0x4000 : 0x0000;
                for(i = 0x20; i <= 0x2F; i++)
                {
