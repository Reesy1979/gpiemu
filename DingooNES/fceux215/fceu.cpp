...
FCEUGI *FCEUI_LoadGameVirtual(const char *name, int OverwriteVidMode)
{
	...
	string *tfilename = new string(fp->filename);
	GameInfo->filename = (char*)tfilename->c_str();
	
	
	if(fp->archiveFilename != "")
	{
		string *tarchivefilename = new string(fp->archiveFilename);
		GameInfo->archiveFilename = (char*)tarchivefilename->c_str();
	}
...
void FCEUI_Emulate(uint8 **pXBuf, int32 **SoundBuf, int32 *SoundBufSize, int skip)
{
	...
	{
		uint32 *zapper = NULL;
		if(joyports[0].type == SI_ZAPPER) zapper = (uint32 *)joyports[0].ptr;
		else
		if(joyports[1].type == SI_ZAPPER) zapper = (uint32 *)joyports[1].ptr;

		extern void FCEU_DrawGunSight(uint8 *buf, int xc, int yc);
		if(zapper) FCEU_DrawGunSight(XBuf, zapper[0], zapper[1]);
	}
	*pXBuf = skip ? 0: XBuf;
	*SoundBuf = WaveFinal;
	*SoundBufSize = ssize;
...
void UpdateAutosave(void)
{
	...
	if(++AutosaveCounter >= AutosaveFrequency)
	{
		...
		f = FCEU_MakeFName(FCEUMKF_AUTOSTATE,AutosaveIndex,0).c_str();
		FCEUSS_Save(f);
