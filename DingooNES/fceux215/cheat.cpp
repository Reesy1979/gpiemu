...
void FCEU_LoadGameCheats(FILE *override)
{
	...
	{
		fn=FCEU_MakeFName(FCEUMKF_CHEAT,0,0).c_str();
...
#else
void FCEU_FlushGameCheats(FILE*, int) {}
#endif
...
{
	int addr, val, compare;
	char *namebuf;

	if(FCEUI_DecodeGG(code, &addr, &val, &compare)) {
		if(namebuf = (char *)FCEU_dmalloc(strlen(name) + 1)) {
			strcpy(namebuf, name);
			if(AddCheatEntry(namebuf, addr, val, compare, 0, 1))
				return 1; 
		}
	}
