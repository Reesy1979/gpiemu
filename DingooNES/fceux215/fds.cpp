...
void FCEU_FDSSelect(void)
{
	...
	extern void ShowMessage(char *format, int disppos = 0, ...);
	ShowMessage("Disk %d Side %c Selected", 0, SelectDisk >> 1, (SelectDisk & 1) ? 'B' : 'A');
#else
...
int FDSLoad(const char *name, FCEUFILE *fp)
{
	...
	fn = (char*)FCEU_MakeFName(FCEUMKF_FDSROM,0,0).c_str();
	...
	{
		FCEUFILE *tp;
		char *fn=(char*)FCEU_MakeFName(FCEUMKF_FDS,0,0).c_str();
